...
class MessageLog:
    ...
    def display_messages(self, y, panel):
        ...
        for message in self.messages:
            # If the current height + upcoming lines wouldn't exceed the log's height, draw the lines
            if y + len(message.lines) < panel.height:
...
def msgbox(text, title=None, width=0, text_color=colors.white, border_color=settings.PANELS_BORDER_COLOR_ACTIVE):
    ...
    gv.root.blit(panel, settings.SCREEN_WIDTH // 2 - panel.width // 2, settings.SCREEN_HEIGHT // 2 - panel.height // 2,
