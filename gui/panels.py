...
def draw_stat_panel(panel, root):
    ...
    render_bar(1, 6, panel, settings.BAR_WIDTH, 'PWR', gv.player.power, gv.player.max_power, colors.black,
               colors.black)
    render_bar(1, 8, panel, settings.BAR_WIDTH, 'DEF', gv.player.defense, gv.player.max_defense, colors.black,
               colors.black)
...
def draw_spotteditems_panel(panel):
    ...
    if len(spotted) > 0:  # if more than one object is present, output the names as a message
        ...
        y += 2
        # Go through the object names and wrap them according to the panel's width
        for obj in spotted:
            line_wrapped = wrap(obj, panel.width - 3)
            # As long as we don't exceed the panel's height, draw the items
            if y + len(line_wrapped) < panel.height - 2:
                for text in line_wrapped:
                    panel.draw_str(x, y, text.title(), bg=None, fg=colors.white)
                    y += 1
            # otherwise draw a line to indicate there's more than can be displayed
            else:
    ...
    if len(gv.player.inventory) > 0:
        ...
        for i in range(len(gv.player.inventory)):
            ...
            if len(text) > 1:  # if the text was wrapped
                # draw subsequent lines in the following line (NOTE: should I add very long item names, this will need to be tweaked)
                for line in text[1:]:
